...
class FetchHandler(session: IMAPSession, tag: Tag) extends BaseActor {
  ...
  override def receive: Receive = {
    case Command.Fetch(userId, sequence, attributes) =>
      val listener = new FetchListener(self, sender, logger, userId)
...
private[command] class FetchListener(self: ActorRef, sender: ActorRef, logger: Logger, userId: UserId) extends IMAPCommandListener {
