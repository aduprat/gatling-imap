...
class StoreHandler(session: IMAPSession, tag: Tag) extends BaseActor {
  ...
  override def receive: Receive = {
    case Command.Store(userId, sequence, flags) =>
      val listener = new StoreListener(self, sender, logger, userId)
...
private[command] class StoreListener(self: ActorRef, sender: ActorRef, logger: Logger, userId: UserId) extends IMAPCommandListener {
